...
abstract class Modelo
{
    ...
    protected function slug()
    {
        $slugBase = $this->slug;
        $sufixo = 1;
        while($this->checarSlugExistente($slugBase)){
            $this->slug = "{$slugBase}-{$sufixo}";
            $sufixo++;
        }
    }

    private function checarSlugExistente(string $slug):bool
    {
        return $this->busca("slug = :s AND id != :id", "s={$this->slug}&id={$this->id}")->total();
    }
